...
- [Передаем данные с клиента на сервер](#derived-props)
...
<a name="derived-props"></a>
## Передаем данные с сервера на клиент
Для того, чтобы передавать данные с сервера на клиент, мы используем Stores и механизм Delivery.

```typescript jsx
import { inject } from 'inversify';
import { DerivedProps } from '../../core/delivery'; // Индентификатор доставляемых свойств
import { makeStore, withServerAction } from '../../core/provider';
import { INextFreeContext } from '../../types/nextFreeContext';

// в этом интерфейсе описываем свойства, которые передаем с сервера на клиент
interface IDerivedData {
  userAgent: string;
}

@makeStore(ExampleStore)
@withServerAction() // регистрация в сервисе Delivery
export class ExampleStore {
  // статический метод; поведение аналогично getInitialProps в pages
  public static async serverAction({ req }: INextFreeContext) {
    return { userAgent: req.headers['user-agent'] }; // свойства который хотим передать на клиент
  }

  //                              _____ Symbol - идентификатор свойств используется, чтобы DI понял,
  //                             /               что мы хотим передать именно результат serverAction в конструктор
  public constructor(@inject(DerivedProps) derivedProps: IDerivedData ) {
    // исполняется и на сервере и на клиенте; derivedProps доступны всегда
    console.log(derivedProps.userAgent);
  }
}
```
**Как работает Delivery**
- Декоратор withServerAction регистрирует метод serverAction из ExampleStore в классе Delivery;
- В файле pages/_app.tsx у класса Delivery вызывается метод execute;
- Исполняются все serverActions во всех сторах;
- Результаты всех методов serverAction передаются на клиент при помощи next/pageProps
- В файле pages/_app.tsx в конструкторе все полученные свойства регистрируются в DI
- Свойства регистрируются только для того класса, для которого был вызван метод serverAction
т.е. свойство userAgent из примера выше не будет видно в других классах.

**С сервера на клиент можно передать только то, что может обработать JSON.parse - нельзя передать функцию, класс
или код.**

### Серверные редиректы при помощи Delivery
Так как в методе serverAction у нас есть доступ к http response, мы можем осуществить
серверный редирект или подставить заголовок и т.д.
...
@makeStore(ExampleStore) // не важно будет это Store или класс специально для роутинга
@withServerAction() // регистрация в сервисе Delivery
export class ExampleStore {
  // статический метод, поведение аналогично getInitialProps в pages
  public static async serverAction({ res, path }: INextFreeContext) {
    if (path !== '/example' && res) {
      res.writeHead(301, { location: '/other/url' });
      return res.end();
    }
    return {}; // не забываем вернуть объект!
  }
}
```
Важно, что для роутинга мы не обязаны использовать стор! Мы можем использовать любой класс, главное
пременить к нему декоратор withServerAction, и пометить как provide() или injectable(). Все зависит от задачи. 
